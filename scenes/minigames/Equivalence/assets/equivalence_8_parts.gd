...
@export var scene_8_pizza: PackedScene  # opcional, solo si prefieres instanciar desde script
@export var allowed_denominators: Array = [2, 4, 8, 16]  # denominadores permitidos
...
func _ready() -> void:
	...
	if pizza_node == null:
		push_warning("No se encontró '8parts_pizza' como hijo del nodo Equivalence. Comprueba el nombre o asigna scene_8_pizza.")
		return
...
func generate_fraction() -> void:
	...
	if denominator==16:
		numerator = rng.randi_range(1, 8)*2
	else:	
...
func _find_pizza_node() -> Node:
	# Primer intento: hijo directo "8parts_pizza"
	if has_node("8parts_pizza"):
		return get_node("8parts_pizza")

	# Búsqueda recursiva por nombre
	var found := _find_node_by_name_recursive(self, "8parts_pizza")
	if found:
		return found

	# Si no aparece, y hay PackedScene asignada, la instanciamos (opcional)
	if scene_8_pizza != null:
		var inst := scene_8_pizza.instantiate()
		add_child(inst)
		return inst
...
# --- Actualizar la interfaz de la FractionCard --- #
...
# --- Botones: recolección directa (hijos directos) --- #
func _setup_buttons_for_pizza(pizza_node: Node) -> void:
	var buttons: Array = []
	_gather_texture_buttons_direct(pizza_node, buttons)

	if buttons.size() == 0:
		push_warning("No se encontraron TextureButton directos dentro de '8parts_pizza'. Comprueba la estructura.")
		return

	# Ordenar por nombre para lectura determinista (TextureButton1..8)
	...
	# Estado inicial: imprimir cuántos están activos y validar la fracción
	...
	# Validación inicial en consola (emitirá señal si es correcto)
	_validate_fraction_against_pizza(pizza_node)


func _gather_texture_buttons_direct(node: Node, out_array: Array) -> void:
	# Solo hijos directos (evita duplicados por jerarquías complejas)
	for child in node.get_children():
...
# Firma: toggled(bool pressed, ...) -> con Callable.bind(pizza_node, btn) recibimos: pressed, pizza_node, btn
