...
func _draw():
	...
	var denominador_pizza = int(original_fraction.denominator)
	var n = int(original_fraction.numerator)
	
	if chosen_fraction:
		denominador_pizza = int(reduced_fraction.denominator)
		n = int(reduced_fraction.numerator)
		
	for i in range(original_fraction.denominator):
		
		var a0 = -PI * 0.5 + two_pi * (i / float(denominador_pizza))
		var a1 = -PI * 0.5 + two_pi * ((i + 1) / float(denominador_pizza))
		var angle_span = a1 - a0
		
		# choose segments to approximate the arc (higher -> smoother)
		var segs = max(4, int(ceil(8.0 * angle_span / (TAU) * denominador_pizza)))
	...
	for i in range(denominador_pizza):
		var a = -PI * 0.5 + two_pi * (i / float(denominador_pizza))
		var p = center + Vector2(cos(a), sin(a)) * radius
		draw_line(center, p, stroke_color, stroke_width)
		
	var circle_segs = max(32, denominador_pizza * 8)
